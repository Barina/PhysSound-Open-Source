...
namespace PhysSound
{
    public class PhysSoundTypeEditor : EditorWindow
    {
        ...
        void SaveTypes()
        {
            ...
            sw.WriteLine("        public static string GetKey(int index) => (index >= PhysSoundTypes.Length) || (index < 0) ? \"\" : PhysSoundTypes[index];");
            sw.WriteLine("");

            sw.WriteLine("        public static bool HasKey(int index) => index < PhysSoundTypes.Length && index >= 0;");
        ...
        void SavePoolSize()
        {
            ...
            {
                Debug.LogError("Could not find PhysSoundTempAudioPool.cs asset. Please re-import the package.");
                return;
            }
